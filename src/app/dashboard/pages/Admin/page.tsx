...
interface User {
  ...
  joiningdate: string;
  status: "Pending" | "Joined";
  type: "Domain Expert" | "ADMIN" | "In-House Team" | "Agent Trainer";
} 
...
export default function AdminPage() {
    ...
    onValue(usersRef, (snapshot) => {
      ...
      if (data) {
        const userList: User[] = Object.keys(data).map((key, index) => ({
          ...
          joiningdate: data[key].joiningdate || "-",
          status: data[key].status as "Pending" | "Joined",
          type: data[key].type as "Domain Expert" | "ADMIN" | "In-House Team" | "Agent Trainer",
        }));
        setUsers(userList);
      }
    });
  ...
  const handleTypeChange = async (userId: string, newType: "Domain Expert" | "ADMIN" | "In-House Team" | "Agent Trainer") => {
    try {
      await update(ref(database, `invitedUsers/${userId}`), {
        type: newType,
      });
                ...
                <tr key={user.id}>
                  ...
                  <td>
                    <select
                      value={user.type}
                      onChange={(e) =>
                        handleTypeChange(user.id, e.target.value as "Domain Expert" | "ADMIN" | "In-House Team" | "Agent Trainer")
                      }
                    >
                      <option value="Domain Expert">Domain Expert</option>
                      <option value="ADMIN">ADMIN</option>
                      <option value="In-House Team">In-House Team</option>
                      <option value="Agent Trainer">Agent Trainer</option>
                    </select>
                  </td>
                  <td>
                    <button
